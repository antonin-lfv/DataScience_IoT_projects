...
st.title("Aperçu des déformations de la structure")
st.divider()
figure = st.empty()
...
# On teste si la collection sensor_data existe, si elle n'existe pas cela veut dire que c'est la première fois
# que les capteurs sont connectés, on ne fait donc rien à part attendre que d'autres données arrivent
if 'sensor_data' in db.list_collection_names():
    print("La collection history_positions existe déjà, on calcule donc les déformations")
    # On a déjà récupéré les données des capteurs, donc on peut calculer la différence d'accélération et de gyroscope
    # avec les données précédentes pour chaque capteur pour ne garder que les données qui ont changé
    ...
    # Get initial accelerations and gyro data from the database deformation_structure_db and collection sensor_data
    # Select the one with the most recent timestamp
    initial_data = db.sensor_data.find().sort('timestamp', 1).limit(1)[0]
    initial_accel_data = initial_data['acceleration']
    initial_gyro_data = initial_data['gyroscope']

    # Get the current acceleration and gyro data
    data = db.sensor_data.find().sort('timestamp', -1).limit(1)[0]
    current_accel_data = data['acceleration']
    current_gyro_data = data['gyroscope']
    timestamp = data['timestamp']

    # Compute the difference in acceleration and gyroscope data
    accel_data = {k: {k2: v2 - initial_accel_data[k][k2] for k2, v2 in v.items()}
                  for k, v in current_accel_data.items()}
    gyro_data = {k: {k2: v2 - initial_gyro_data[k][k2] for k2, v2 in v.items()}
                 for k, v in current_gyro_data.items()}
    ...
    if last_timestamp != last_timestamp_history:
        # Save the new positions to mongoDB in the database deformation_structure_db and the
        # collection history_positions
        db = client.deformation_structure_db
        ...
        collection.insert_one({"timestamp": timestamp,
                               "positions": {key: list(val) for key, val in
                                             zip(initial_positions.keys(), new_positions)}})

else:
    print("Initialisation des capteurs en cours...")
    # We juste plot the initial structure
    initial_positions = db.history_positions.find().sort('timestamp', -1).limit(1)[0]
    initial_positions = {k: a(v) for k, v in initial_positions['positions'].items() if k != '_id'}
    data = [go.Scatter3d(x=[initial_positions[i][0] for i in initial_positions.keys()],
                         y=[initial_positions[i][1] for i in initial_positions.keys()],
                         z=[initial_positions[i][2] for i in initial_positions.keys()],
                         mode="lines+markers",
                         opacity=1,
